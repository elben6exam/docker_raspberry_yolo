...
OPENCV_EXTRA=$DEV_HOME/opencv_contrib

MNN_HOME=$DEV_HOME/MNN
ncnn_HOME=$DEV_HOME/ncnn
...
checkSrcTree $OPENCV_EXTRA https://github.com/opencv/opencv_contrib.git 4.x
...
checkSrcTree $MNN_HOME https://github.com/alibaba/MNN.git master
checkSrcTree $ncnn_HOME https://github.com/Tencent/ncnn.git master
...
mkdir -p $MNN_HOME/build && \
cd $MNN_HOME/build && \
cmake -DMNN_BUILD_DEMO=ON \
      $MNN_HOME && \
make -j$BUILD_JOBS && \
cmake -DCMAKE_INSTALL_PREFIX=$STAGING_DIR/extras/MNN -P cmake_install.cmake && \
echo export MNN_DIR=\$INSTALLDIR/extras/MNN > $STAGING_DIR/extras/MNN/setupvars.sh && \
echo export LD_LIBRARY_PATH=\$INSTALLDIR/extras/MNN/lib:\$LD_LIBRARY_PATH >> $STAGING_DIR/extras/MNN/setupvars.sh && \
cd $DEV_HOME || fail 14 "MNN build failed. Stopping"

#Build ncnn
mkdir -p $ncnn_HOME/build && \
cmake $MNN_HOME && \
make -j$BUILD_JOBS && \
cmake -DCMAKE_INSTALL_PREFIX=$STAGING_DIR/extras/ncnn -P cmake_install.cmake && \
export ncnn_DIR=$STAGING_DIR/extras/ncnn/lib/cmake/ncnn > $STAGING_DIR/extras/ncnn/setupvars.sh &&\
echo export LD_LIBRARY_PATH=\$INSTALLDIR/extras/ncnn/lib:\$LD_LIBRARY_PATH >> $STAGING_DIR/extras/ncnn/setupvars.sh && \
cd $DEV_HOME || fail 15 "ncnn build failed. Stopping"

#Build OpenVINO
mkdir -p $OPENVINO_HOME/build && \
cd $OPENVINO_HOME/build && \
cmake -DOpenCV_DIR=$STAGING_DIR/extras/opencv/cmake -DENABLE_OPENCV=OFF \
      -DPYTHON_INCLUDE_DIRS="/opt/python3.8_arm/include/python3.8" \
      -DPYTHON_LIBRARY="/opt/python3.8_arm/lib/libpython3.8.so" \
      -DENABLE_PYTHON=ON \
...
cmake -DInferenceEngineDeveloperPackage_DIR=$OPENVINO_HOME/build \
      -DENABLE_PYTHON=ON -DPYTHON_EXECUTABLE="/opt/python3.8_arm/bin/python3.8" \
      -DPYTHON_INCLUDE_DIRS=/opt/python3.8_arm/include/python3.8 \
      -DPYTHON_LIBRARIES=/opt/python3.8_arm/lib \
      -DPYTHON_MODULE_EXTENSION=".so" \
